...
class PatchMaskCB():
    ...
    def patch_masking(self, xb):
        """
        xb: [bs x seq_len x n_vars] -> [bs x num_patch x n_vars x patch_len]
        """
        xb_patch, num_patch = create_patch(xb, self.patch_len,
...
if __name__ == "__main__":
    bs, L, nvars,D = 32, 6,16, 7
    xb = torch.randn(bs, L, nvars,D)
    x_masked, x_kept, mask, ids_restore = random_masking(xb, 0.7)
    a = loss2(xb, x_masked, mask)
